defmodule NewRelic.Transaction do
  ...
  def start(name) when is_binary(name) do
    %__MODULE__{name: name, start_time: now()}
  end
  ...
  def finish(%__MODULE__{start_time: start_time}=transaction) do
    ...
    record(transaction, %TransactionEvent{category: :total}, elapsed)
  end

  @spec record(t, TransactionEvent.t, non_neg_integer()) :: nil
  def record(%__MODULE__{name: name}, %TransactionEvent{category: category} = event, elapsed) when category in [:total, :event, :db, :ext] do
    NewRelic.Collector.record_value(name, event, elapsed)
  end

  @spec record(t, TransactionError.t) :: nil
  def record(%__MODULE__{name: name}, %TransactionError{}=error) do
    NewRelic.Collector.record_error(name, error)
  end
